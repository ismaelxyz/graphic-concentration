...
def LoadTexture(path):
    ...
    if ((WidthPixels > glMaxTexDim) or (HeightPixels > glMaxTexDim)):
        ...
        raise RuntimeError("Texture image (%d by %d) is larger than supported by GL %d." % (
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 3, WidthPixels, HeightPixels,
...
def InitGL(Width, Height):
    ...
    status, gCamera.m_GlowTexture = LoadTexture(
        os.path.join("Art", "HardGlow2.bmp"))
    if (not status):
    ...
    status, gCamera.m_BigGlowTexture = LoadTexture(
        os.path.join("Art", "BigGlow3.bmp"))
    if (not status):
    ...
    status, gCamera.m_HaloTexture = LoadTexture(
        os.path.join("Art", "Halo3.bmp"))
    if (not status):
    ...
    status, gCamera.m_StreakTexture = LoadTexture(
        os.path.join("Art", "Streaks4.bmp"))
    if (not status):
    ...
    gStartTime = time.time()                     # // Get the time the app started
...
def DrawGLScene():
    ...
    glRotatef((time.time() * 1000.0) / 50.0, 0.3, 0.0, 0.0)
    glRotatef((time.time() * 1000.0) / 50.0, 0.0, 0.5, 0.0)
    glCallList(gcylList)
    # glPopMatrix();
