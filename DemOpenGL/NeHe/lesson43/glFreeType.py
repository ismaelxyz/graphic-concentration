...
def is_font_available(ft, facename):
    """ Returns true if FreeType can find the requested face name 
        Pass the basname of the font e.g. "unifont" or "times new roman"
    """
...
class font_data:
    def __init__(self, facename, pixel_height):
        ...
        try:
            ...
            raise ValueError(
                "Unable to locate true type font '%s'" % (facename))
...
def main():
    print('testing availability of freetype font unifont\n')
    ft = ImageFont.truetype('unifont.ttf', 15)
    if ft:
        print("Found the TrueType font 'unifont.ttf'")
    else:
        print("faild to find the TrueTYpe font 'unifont'\n")
