...
class Surface:
    ...
    def __init__(self, facefile="1crn.face", vertfile="1crn.vert", pdbfile="1crn.pdb"):
        ...
        alpha = 1
        self.colordict = {
            'C': [.5, .5, .5, alpha], 'O': [1, 0, 0, alpha],
            'N': [0, 0, 1, alpha], 'S': [1, 1, 0, alpha],
            'P': [1, 0, 1, alpha], 'H': [1, 1, 1, alpha],
            'U': [0, 0, 0, alpha]
        }

        self.mapdict = {'C': 0.5, 'O': .9, 'N': 0.1, 'S': 0.5, 'P': 0.5, 'H': 0.5,
                        'U': 0.5}
        self.surface = None
        self.bond = None
        self.SetupWindow()
    ...
    def SetupBonds(self):
        ...
        for i in self.topol:
            ...
            glVertex3f(self.crd[at1][0], self.crd[at1][1], self.crd[at1][2])
            color = self.colorlist2[at2]
            apply(glColor3f, tuple(color))
            glVertex3f(self.crd[at2][0], self.crd[at2][1], self.crd[at2][2])
    ...
    def ReadSurface(self):
        ...
        self.vert = zeros((vertices, 3), 'f')
        self.norm = zeros((vertices, 3), 'f')
        self.nearest = zeros((vertices))
        ...
        vcen = add.reduce(self.vert)/len(self.vert)
