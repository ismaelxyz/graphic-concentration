...
class PDBRecord:
    def __init__(self, xtype=None, anum=None, atom=None, residue=None,
                 chain=None, rnum=None):
    ...
    def Read(self, filename):
        ...
        while (1):
            ...
            if line[0:4] == 'ATOM' or line[0:6] == 'HETATM':
                ...
                self.records.append(PDBRecord(xtype, anum, atom, residue, chain,
                                              rnum))
                self.crds.append((x, y, z))

        self.crds = array(self.crds)
    ...
    def Write(self, filename):
        ...
        for i in range(len(self.records)):
            ...
            self.pdbfd.write('%-6s%5d %-4s%c%-4s%c%4d%c   %8.3f%8.3f%8.3f\n' %
                             (record.type, record.anum, record.atom, ' ',
                              record.residue, ' ', record.rnum, ' ',
    ...
    def ReturnAnum(self, atom, rnum):
        for i in range(len(self.records)):
            ...
            if record.atom == atom and record.rnum == rnum:
                return i
        sys.stderr.write(
            'Unable to find atom "%s" in residue %s\n' % (atom, rnum))
        return None
