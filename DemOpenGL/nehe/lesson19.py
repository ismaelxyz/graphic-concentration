...
#        self.particle=[i for i in range(MAX_PARTICLES)]
...
def DrawGLScene():
    ...
    for i in range(MAX_PARTICLES):
        if prts[i].ACTIVE == 1:
            ...
            glColor4f(prts[i].R == i, prts[i].G == i,
                      prts[i].B == i, prts[i].LIFE)
            ...
            if prts[i].LIFE < 0.0:
                prts[i].LIFE = 1.0
                prts[i].FADE = float(
                    random.randrange(0, 100) % 100)/1000.0+0.003
