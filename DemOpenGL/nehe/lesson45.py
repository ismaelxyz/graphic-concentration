...
class CMesh:
    ...
    def LoadHeightmap(self, szPath, flHeightScale, flResolution):
        ...
        self.m_nVertexCount = int(
            sizeX * sizeY * 6 / (flResolution * flResolution))
        # self.m_pVertices = zeros ((self.m_nVertexCount * 3), 'f')             # // Vertex Data
        # Non strings approach
        self.m_pVertices = zeros(
            (self.m_nVertexCount, 3), 'f')             # // Vertex Data
        # // Texture Coordinates
        self.m_pTexCoords = zeros((self.m_nVertexCount, 2), 'f')
        ...
        self.m_nTextureID = glGenTextures(
            1)                        # // Get An Open ID
        # // Bind The Texture
    ...
    def BuildVBOs(self):
        ...
        if (g_fVBOSupported):
            ...
            glBufferDataARB(GL_ARRAY_BUFFER_ARB,
                            self.m_pVertices, GL_STATIC_DRAW_ARB)
            ...
            # // Load The Data
            glBufferDataARB(GL_ARRAY_BUFFER_ARB,
                            self.m_pTexCoords, GL_STATIC_DRAW_ARB)
...
def IsExtensionSupported(TargetExtension):
    ...
    if found_extension == False:
        ...
        print("OpenGL rendering context does not support '%s'" %
              (TargetExtension))
        return False
...
def DrawGLScene():
    ...
    if (g_fVBOSupported):
        ...
        # Also see OpenGLContext Working with numpy Python
